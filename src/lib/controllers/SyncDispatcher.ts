...
export class SyncDispatcher {
    ...
    // src/lib/controllers/SyncDispatcher.ts refinements

    // 1. Defensive Mutation Mapping
    addMutations(
        id: string,
        ...
        // If mutations have different colors, they should probably be separate rules, 
        // or the ColorRule type needs to support a map of positions to colors.
        // Currently, we'll assume a single-color rule but add a check:
        ...
        this.dispatch({
            ...
            rule: {
                ...
                residues,
                color: mutations[0].color, // Logic check: ensure this is intended
            }
        });
    }

    // 2. Explicit Sync in removal to match the "Clear" logic
    private handleRemoveColorRule(id: string): void {
        const removed = this.colorManager.removeRule(id);
        if (removed) {
            this.syncColors();
        }
    }
