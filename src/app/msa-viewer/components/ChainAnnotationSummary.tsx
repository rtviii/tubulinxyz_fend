...
export function ChainAnnotationSummary({ rcsb_id, auth_asym_id, onAnnotationsLoaded }: ChainAnnotationSummaryProps) {
const hasNotified = useRef(false); // Track if we've already notified
  ...
  useEffect(() => {
    // Only notify once when data first arrives
    if (data && onAnnotationsLoaded && !hasNotified.current) {
      onAnnotationsLoaded(data);
      hasNotified.current = true;
    }
  }, [data]); // Remove onAnnotationsLoaded from deps
  if (isLoading) {
    return (
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 flex flex-wrap gap-1">
            {mutations.slice(0, 5).map((mut: any, i: number) => (
              <span key={i} className="px-1.5 py-0.5 bg-red-50 text-red-700 rounded font-mono text-xs">
